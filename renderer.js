...
function addJumpToIE () {
        let url = view.src;
        let ieurl = new IEUrl(uuid.v1(), url);
        jsonfile.readFile(ielinks, function(err, curr) {
            curr.push(ieurl);
            jsonfile.writeFile(ielinks, curr, function (err) {
            });
        });
}
...
jumpy.addEventListener('click', addJumpToIE);